...
class TransactionAccess(object):
    ...
    Wraps a standard notifier to provide transaction support over multiple
    access.N calls, where N is a specific service.

    Provide a notifier/publisher wrapper which splits access calls into
    two component parts; 1) an initiation component which will locally
    save each request and return a tracking sequence number(s) for the
    request(s),    2) a commit component which will send the requests,
    specified by tracking sequence number(s), to the service, execute
    them in a single transaction, and wait for and return responses.

    The intention is to provide a mechanism where multiple access calls
    for a given service can be combined and executed transactionally,
    allowing a failure in and single call to terminate the entire
    transaction.

    NOTE: This wrapper is NOT intended as a mechanism to improved access
          call performance. For a wrapper to improve access performance
          see SplitAccess.

    See: graph.TransactionAccess(), channel.TransactionAccess()
    '''
    ...
    def get_service(self):
        ...
        Return the service on which the transactions will be performed.

        NOTE: This class is intended to be subclassed by a service which
              defines this value.
        '''      
        raise error.NoServiceDefined('no service defined')

    def rpcs(self, obj, vid_list, cmd, args, *more, **kwargs):
        ret = []
        for vid in vid_list:
            task = (vid, args['command'], args['args'], args['kwargs'])
            self._seq += 1
            self._transaction[self._seq] = task
            ret.append([self._seq])

        return ret

    @_complete_trace_decorator()
    def commit(self, seq_list):
        '''commit

        Given a list of sequence numbers, produced by access requests
        using this object as notifier, send those same requests to a
        service for execution in a single DB level transaction, wait
        for the request completions and return the result(s) as a list
        in the same order as the requested sequence numbers.

        Examples:

          [r1, r2, r3, r4] = self.commit([s1, s2, s3, s4])
        '''

        tasks = [self._transaction[seq] for seq in seq_list]
        ret = _random(
            self.get_service(),
            self._notifier,
            '_transaction',
